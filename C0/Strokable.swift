...
final class StrokableSender: SubSender {
    ...
    func send(_ actionMap: ActionMap, from sender: Sender) {
            ...
            if let eventValue = actionMap.eventValuesWith(DragEvent.self).first {
                if actionMap.phase == .began,
                    let receiver = sender.mainIndicatedView
                        .withSelfAndAllParents(with: Receiver.self) {
                    
                    let viewStroker = receiver.makeViewStroker()
                    self.stroker = Stroker(viewStroker: viewStroker)
                }
                guard let stroker = stroker else { return }
                let p = stroker.viewStroker.view.convertFromRoot(eventValue.rootLocation)
                stroker.stroke(for: p, pressure: eventValue.pressure,
                               time: eventValue.time, actionMap.phase,
                               to: sender.indicatedVersionView.version)
        ...
        case actionList.lassoEraseAction:
            if let eventValue = actionMap.eventValuesWith(DragEvent.self).first {
                ...
                guard let stroker = stroker else { return }
                let p = stroker.viewStroker.view.convertFromRoot(eventValue.rootLocation)
                stroker.lassoErase(for: p, pressure: eventValue.pressure,
                                   time: eventValue.time, actionMap.phase)
                if actionMap.phase == .ended {
                    self.stroker = nil
                }
    ...
    final class Stroker {
        var viewStroker: ViewStroker
        
        init(viewStroker: ViewStroker) {
            self.viewStroker = viewStroker
        }
        ...
        func stroke(for point: Point, pressure: Real, time: Real, _ phase: Phase,
            ...
            let ap = viewStroker.convertToCurrentLocal(point)
            switch phase {
            case .began:
